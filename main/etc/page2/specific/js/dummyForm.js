fitx.utils.require(['fitx', 'page2', 'displayDummyForm'])
jQuery(window).load(function () {
    jQuery('#dummyForm').hide();
    setupAJAXSubmit('unnecessary', 'newStudentFormAction', setupData, setupConstraints, '.ok', errorFunc, successFunc)
    setupAJAXSubmit('vehicleGroupManagementForm', 'editStudent', setupData2, setupConstraints, '.editbutton', errorFunc, successFunc)
    setupFlexiGrid('#studentTable', undefined, "Welcome Blahblah", undefined, undefined, undefined, undefined, classData)
    sendAjaxRequest('studentData', setupDataFlexi(), showReport)
    ...
    jQuery('.pcontrol input').keypress(function (e) {
        ...
        if (key == 13)  // the enter key code
        {
			...
			if(pageNo>=1 && pageNo<=parseInt(total/rp)+1)
				sendAjaxRequest('studentData', {'pageNo': pageNo}, showReport)
        }
    })
...
var setupData = function () {
    ...
    data.studentName = jQuery('#studentName').val()
    data.date = jQuery('#date').val()
	data.section = jQuery('#section :selected').val()
...
function showReport(result) {
    ...
    jQuery('#studentTable').flexAddData(result.message.sendData)
...
function onFirstPageRequest() {
    sendAjaxRequest('studentData', {'pageNo': 1}, showReport)
}
...
function onRpChange() {
	...
	sendAjaxRequest('studentData',{'pageNo':1,'rp':rp}, showReport)
}
    ...
    if (com == "Edit") {
        ...
        if (c == 1) {
            ...
            jQuery('.trSelected', grid).each(function () {
                ...
                editFields.push({'studentName': jQuery('td[abbr="Student Name"] >div', this).html()})

                editFields.push({'section': jQuery('td[abbr="Section"] >div', this).html()})

                var div = jQuery('#dummyForm')
				...
				var sex = jQuery('td[abbr="Sex"] >div', this).html ();
				div.find('input[value=' + sex + ']').attr('checked','checked');

				var date = jQuery('td[abbr="Date"] >div', this).html ();
				div.find('#date').val(date);

                idToEdit=jQuery('td[abbr="Id"] >div', this).html()
                jQuery('.userMessage').text(" ");
                jQuery("#tableDiv").toggle('showOrHide')
                jQuery('#dummyForm').toggle('showOrHide')
