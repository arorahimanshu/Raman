...
(function (jQuery) {
    ...
    if(typeof jQuery.support.selectstart != 'function') {
	...
	jQuery.addFlex = function (t, p) {
		...
		p = jQuery.extend({ //apply default properties
		...
		var g = {
			...
			fixHeight: function (newH) {
				...
				var nd = parseInt(jQuery(g.nDiv).height(), 10);
				if (nd > newH) jQuery(g.nDiv).height(newH).width(200);
				else jQuery(g.nDiv).height('auto').width('auto');
				jQuery(g.block).css({
					height: newH,
					marginBottom: (newH * -1)
				});
			...
			dragStart: function (dragtype, e, obj) { //default drag function start
                if (dragtype == 'colresize' && p.colResize === true) {//column resize
                    ...
                    jQuery(e.target).disableSelection(); //disable selecting the column header
                    if((p.colMove === true)) {
                        jQuery(g.nDiv).hide();
                        jQuery(g.nBtn).hide();
			...
			dragEnd: function () {
				if (this.colresize) {
					...
					jQuery('tr', this.bDiv).each(
						function () {
							var jQuerytdDiv = jQuery('td:visible div:eq(' + n + ')', this);
							jQuerytdDiv.css('width', nw);
							g.addTitleToCell(jQuerytdDiv);
						}
					);
					...
					if (jQuery.cookies) {
						...
						jQuery.cookie('flexiwidths/'+name, nw);
					}
					...
					if (this.dcolt !== null) {
						if (this.dcoln > this.dcolt) jQuery('th:eq(' + this.dcolt + ')', this.hDiv).before(this.dcol);
						else jQuery('th:eq(' + this.dcolt + ')', this.hDiv).after(this.dcol);
			...
			toggleCol: function (cid, visible) {
				...
				if (jQuery('input:checked', g.nDiv).length < p.minColToggle && !visible) {
					return false;
				}
				if (visible) {
					ncol.hidden = false;
			...
			switchCol: function (cdrag, cdrop) { //switch columns
				jQuery('tbody tr', t).each(
					function () {
						if (cdrag > cdrop) jQuery('td:eq(' + cdrop + ')', this).before(jQuery('td:eq(' + cdrag + ')', this));
						else jQuery('td:eq(' + cdrop + ')', this).after(jQuery('td:eq(' + cdrag + ')', this));
					}
				);
			...
			addData: function (data) { //parse data
				if (p.dataType == 'json') {
					data = jQuery.extend({rows: [], page: 0, total: 0}, data);
				}
				if (p.preProcess) {
				...
				if (p.dataType == 'json') {
					jQuery.each(data.rows, function (i, row) {
						...
						jQuery('thead tr:first th', g.hDiv).each( //add cell
							function () {
								...
								if (typeof row.cell == 'undefined') {
                                    ...
                                    td.innerHTML = p.__mw.datacol(p, jQuery(this).attr('abbr'), iHTML); //use middleware datacol to format cols
								}
								// If the content has a <BGCOLOR=nnnnnn> option, decode it.
								var offs = td.innerHTML.indexOf( '<BGCOLOR=' );
								if( offs >0 ) {
                                    jQuery(td).css('background', text.substr(offs+7,7) );
								}

								jQuery(td).attr('abbr', jQuery(this).attr('abbr'));
								jQuery(tr).append(td);
								td = null;
							}
						);
					...
					jQuery("rows row", data).each(function () {
						i++;
						var tr = document.createElement('tr');
						if (jQuery(this).attr('name')) tr.name = jQuery(this).attr('name');
						if (jQuery(this).attr('color')) {
							jQuery(tr).css('background',jQuery(this).attr('id'));
						} else {
							if (i % 2 && p.striped) tr.className = 'erow';
						}
						...
						jQuery('thead tr:first th', g.hDiv).each(function () {
							...
							var text = jQuery("cell:eq(" + idx + ")", robj).text();
							var offs = text.indexOf( '<BGCOLOR=' );
							if( offs >0 ) {
								jQuery(td).css('background',	 text.substr(offs+7,7) );
							}
                            td.innerHTML = p.__mw.datacol(p, jQuery(this).attr('abbr'), text); //use middleware datacol to format cols
							jQuery(td).attr('abbr', jQuery(this).attr('abbr'));
							jQuery(tr).append(td);
							td = null;
						});
			...
			populate: function () { //get latest data
				...
				if (p.hideOnSubmit) {
					jQuery(this.gDiv).prepend(g.block);
				}
				if (browser.opera) {
			...
			changePage: function (ctype) { //change page
				...
				switch (ctype) {
					...
					case 'last':
						...
						break;
					case 'input':
						var nv = parseInt(jQuery('.pcontrol input', this.pDiv).val(), 10);
						if (isNaN(nv)) {
							nv = 1;
						}
						if (nv < 1) {
			...
			getCellDim: function (obj) {// get cell prop for editable event
				var ht = parseInt(jQuery(obj).height(), 10);
				var pht = parseInt(jQuery(obj).parent().height(), 10);
				var wt = parseInt(obj.style.width, 10);
				var pwt = parseInt(jQuery(obj).parent().width(), 10);
				var top = obj.offsetParent.offsetTop;
				var left = obj.offsetParent.offsetLeft;
				var pdl = parseInt(jQuery(obj).css('paddingLeft'), 10);
				var pdt = parseInt(jQuery(obj).css('paddingTop'), 10);
				return {
					ht: ht,
					wt: wt,
					top: top,
			...
			addRowProp: function () {
				jQuery('tbody tr', g.bDiv).on('click', function (e) {
			...
			addTitleToCell: function(tdDiv) {
				if(p.addTitleToCell) {
					var jQueryspan = jQuery('<span />').css('display', 'none'),
						jQuerydiv = (tdDiv instanceof jQuery) ? tdDiv : jQuery(tdDiv),
						div_w = jQuerydiv.outerWidth(),
						span_w = 0;

					jQuery('body').children(':first').before(jQueryspan);
					jQueryspan.html(jQuerydiv.html());
					jQueryspan.css('font-size', '' + jQuerydiv.css('font-size'));
					jQueryspan.css('padding-left', '' + jQuerydiv.css('padding-left'));
					span_w = jQueryspan.innerWidth();
					jQueryspan.remove();

					if(span_w > div_w) {
						jQuerydiv.attr('title', jQuerydiv.text());
					} else {
						jQuerydiv.removeAttr('title');
					}
				}
			},
			autoResizeColumn: function (obj) {
				...
				var n = jQuery('div', this.cDrag).index(obj),
					jQueryth = jQuery('th:visible div:eq(' + n + ')', this.hDiv),
					ol = parseInt(obj.style.left, 10),
					ow = jQueryth.width(),
					nw = 0,
					nl = 0,
					jQueryspan = jQuery('<span />');
				jQuery('body').children(':first').before(jQueryspan);
				jQueryspan.html(jQueryth.html());
				jQueryspan.css('font-size', '' + jQueryth.css('font-size'));
				jQueryspan.css('padding-left', '' + jQueryth.css('padding-left'));
				jQueryspan.css('padding-right', '' + jQueryth.css('padding-right'));
				nw = jQueryspan.width();
				jQuery('tr', this.bDiv).each(function () {
					var jQuerytdDiv = jQuery('td:visible div:eq(' + n + ')', this),
						spanW = 0;
					jQueryspan.html(jQuerytdDiv.html());
					jQueryspan.css('font-size', '' + jQuerytdDiv.css('font-size'));
					jQueryspan.css('padding-left', '' + jQuerytdDiv.css('padding-left'));
					jQueryspan.css('padding-right', '' + jQuerytdDiv.css('padding-right'));
					spanW = jQueryspan.width();
					nw = (spanW > nw) ? spanW : nw;
				});
				jQueryspan.remove();
				nw = (p.minWidth > nw) ? p.minWidth : nw;
				nl = ol + (nw - ow);
				jQuery('div:eq(' + n + ')', this.cDrag).css('left', nl);
				this.colresize = {
					nw: nw,
					n: n
				};
			...
			for (var i = 0; i < p.colModel.length; i++) {
				...
				if( cm ) {	// only use cm if its defined
					if (jQuery.cookies) {
						...
						if( jQuery.cookie(cookie_width) != undefined ) {
							cm.width = jQuery.cookie(cookie_width);
						}
					}
        ...
        if(p.colResize === false) { //don't display column drag if we are not using it
            jQuery(g.cDrag).css('display', 'none');
        }
		...
		if (p.buttons) {
			...
			for (var i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
					...
					if (btn.bclass) jQuery('span', btnDiv).addClass(btn.bclass).css({
						paddingLeft: 20
					});
					if (btn.bimage) // if bimage defined, use its string as an image url for this buttons style (RS)
						jQuery('span',btnDiv).css( 'background', 'url('+btn.bimage+') no-repeat center left' );
						jQuery('span',btnDiv).css( 'paddingLeft', 20 );

					if (btn.tooltip) // add title if exists (RS)
						jQuery('span',btnDiv)[0].title = btn.tooltip;
		...
		if( p.combobuttons && jQuery(g.tDiv2) )
		{
			var btnDiv = document.createElement('div');
			btnDiv.className = 'fbutton';

			var tSelect = document.createElement('select');
			jQuery(tSelect).change( function () { g.combo_doSelectAction( tSelect ) } );
			jQuery(tSelect).click( function () { g.combo_resetIndex( tSelect) } );
			tSelect.className = 'cselect';
			jQuery(btnDiv).append(tSelect);
			...
			for (i=0;i<p.combobuttons.length;i++)
			{
				...
				if (!btn.separator)
				{
					...
					if (btn.bclass)
						...
						;
					if (btn.bimage)  // if bimage defined, use its string as an image url for this buttons style (RS)
						jQuery(btnOpt).css( 'background', 'url('+btn.bimage+') no-repeat center left' );
						jQuery(btnOpt).css( 'paddingLeft', 20 );

					if (btn.tooltip) // add title if exists (RS)
						jQuery(btnOpt)[0].title = btn.tooltip;

					if (btn.onpress)
					{
						btnOpt.value = btn.onpress;
					}
					jQuery(tSelect).append(btnOpt);
				}
			}
			jQuery('.tDiv2').append(btnDiv);
		}
		...
		jQuery('thead tr:first th', g.hDiv).each(function () {
			...
			jQuery(this).empty().append(thdiv).removeAttr('width').mousedown(function (e) {
				g.dragStart('colMove', e, this);
			}).hover(function () {
				if (!g.colresize && !jQuery(this).hasClass('thMove') && !g.colCopy) {
					jQuery(this).addClass('thOver');
				}
				if (jQuery(this).attr('abbr') != p.sortname && !g.colCopy && !g.colresize && jQuery(this).attr('abbr')) {
					jQuery('div', this).addClass('s' + p.sortorder);
				...
				if (g.colCopy) {
					...
					if (n < g.dcoln) {
						jQuery(this).append(g.cdropleft);
					} else {
						jQuery(this).append(g.cdropright);
					}
					g.dcolt = n;
				} else if (!g.colresize) {
					var nv = jQuery('th:visible', g.hDiv).index(this);
					var onl = parseInt(jQuery('div:eq(' + nv + ')', g.cDrag).css('left'), 10);
					...
					var ndw = parseInt(jQuery(g.nDiv).width(), 10);
					jQuery(g.nDiv).css({
						top: g.bDiv.offsetTop
					});
				...
				if (jQuery(this).attr('abbr') != p.sortname) {
					jQuery('div', this).removeClass('s' + p.sortorder);
        ...
        if (p.colResize === true) {
            ...
            if(cdcol !== null) {
                ...
                g.cdpad = 0;
                g.cdpad += (isNaN(parseInt(jQuery('div', cdcol).css('borderLeftWidth'), 10)) ? 0 : parseInt(jQuery('div', cdcol).css('borderLeftWidth'), 10));
                g.cdpad += (isNaN(parseInt(jQuery('div', cdcol).css('borderRightWidth'), 10)) ? 0 : parseInt(jQuery('div', cdcol).css('borderRightWidth'), 10));
                g.cdpad += (isNaN(parseInt(jQuery('div', cdcol).css('paddingLeft'), 10)) ? 0 : parseInt(jQuery('div', cdcol).css('paddingLeft'), 10));
                g.cdpad += (isNaN(parseInt(jQuery('div', cdcol).css('paddingRight'), 10)) ? 0 : parseInt(jQuery('div', cdcol).css('paddingRight'), 10));
                g.cdpad += (isNaN(parseInt(jQuery(cdcol).css('borderLeftWidth'), 10)) ? 0 : parseInt(jQuery(cdcol).css('borderLeftWidth'), 10));
                g.cdpad += (isNaN(parseInt(jQuery(cdcol).css('borderRightWidth'), 10)) ? 0 : parseInt(jQuery(cdcol).css('borderRightWidth'), 10));
                g.cdpad += (isNaN(parseInt(jQuery(cdcol).css('paddingLeft'), 10)) ? 0 : parseInt(jQuery(cdcol).css('paddingLeft'), 10));
                g.cdpad += (isNaN(parseInt(jQuery(cdcol).css('paddingRight'), 10)) ? 0 : parseInt(jQuery(cdcol).css('paddingRight'), 10));
		...
		if (p.usepager) {
			...
			if (browser.msie && browser.version < 7) jQuery('.pButton', g.pDiv).hover(function () {
			...
			if (p.searchitems) {
				...
				jQuery('.pSearch', g.pDiv).click(function () {
					jQuery(g.sDiv).slideToggle('fast', function () {
						jQuery('.sDiv:visible input:first', g.gDiv).trigger('focus');
					});
				});
				//add search box
				...
				jQuery(g.sDiv).append("<div class='sDiv2'>" + p.findtext +
						" <input type='text' value='" + p.query +"' size='30' name='q' class='qsbox' /> "+
						" <select name='qtype'>" + sopt + "</select></div>");
				//Split into separate selectors because of bug in jQuery 1.3.2
				jQuery('input[name=q]', g.sDiv).keydown(function (e) {
					if (e.keyCode == 13) {
						g.doSearch();
					}
				});
				jQuery('select[name=qtype]', g.sDiv).keydown(function (e) {
					if (e.keyCode == 13) {
						g.doSearch();
					}
				});
		...
		if (jQuery('th', g.hDiv).length) {
			...
			var cn = 0;
			jQuery('th div', g.hDiv).each(function () {
			...
			if (browser.msie && browser.version < 7.0) jQuery('tr', g.nDiv).hover(function () {
				jQuery(this).addClass('ndcolover');
			}, function () {
				jQuery(this).removeClass('ndcolover');
			});
			jQuery('td.ndcol2', g.nDiv).click(function () {
			...
			jQuery('input.togCol', g.nDiv).click(function () {
		...
		// add date edit layer
		jQuery(g.iDiv).addClass('iDiv').css({
			display: 'none'
		});
		...
		jQuery(g.gDiv).hover(function () {}, function () {
			jQuery(g.nDiv).hide();
			jQuery(g.nBtn).hide();
		});
		//add document events
	...
	jQuery.fn.noSelect = function (p) { //no select plugin by me :-)
		...
		if (prevent) {
			return this.each(function () {
				if (browser.msie || browser.safari) jQuery(this).bind('selectstart', function () {
					return false;
				});
				else if (browser.mozilla) {
					jQuery(this).css('MozUserSelect', 'none');
					jQuery('body').trigger('focus');
				} else if (browser.opera) jQuery(this).bind('mousedown', function () {
					return false;
				});
			...
			return this.each(function () {
				if (browser.msie || browser.safari) jQuery(this).unbind('selectstart');
				else if (browser.mozilla) jQuery(this).css('MozUserSelect', 'inherit');
				else if (browser.opera) jQuery(this).unbind('mousedown');
				else jQuery(this).removeAttr('unselectable', 'on');
			});
		}
	}; //end noSelect
	jQuery.fn.flexSearch = function(p) { // function to search grid
		return this.each( function() { if (this.grid&&this.p.searchitems) this.grid.doSearch(); });
	}; //end flexSearch
	jQuery.fn.selectedRows = function (p) { // Returns the selected rows as an array, taken and adapted from http://stackoverflow.com/questions/11868404/flexigrid-get-selected-row-columns-values
		var arReturn = [];
		var arRow = [];
		var selector = jQuery(this.selector + ' .trSelected');


		jQuery(selector).each(function (i, row) {
			arRow = [];
			var idr = jQuery(row).data('id');
			jQuery.each(row.cells, function (c, cell) {
				var col = cell.abbr;
